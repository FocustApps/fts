...
class ActionStepModel(BaseModel):
    ...
    def validate_action_type(cls, v: str) -> str:
        ...
        if not should_validate_write():
            return v
        valid_types = [
            "api_action", "ui_action", "database_action", 
            "infrastructure_action", "repository_action"
        ]
...
class ActionReferenceCache:
    ...
    @classmethod
    def check_exists(cls, action_type: str, action_id: str, session: Session) -> bool:
        """
    ...
    @classmethod
    def _query_action_exists(cls, action_type: str, action_id: str, session: Session) -> bool:
        ...
        result = session.query(table_class).filter_by(
            **{f"{action_type}_id": action_id}
        ).first()
...
def insert_action_chain(
    action_chain: ActionChainModel, session: Session, engine: Engine
) -> ActionChainModel:
    ...
    with session() as db_session:
        ...
        for step in steps:
            ...
            if use_cache:
                ...
                exists = ActionReferenceCache._query_action_exists(action_type, action_id, db_session)
