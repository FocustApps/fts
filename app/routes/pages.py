...
async def get_pages(request: Request, current_user: TokenPayload = Depends(get_current_user)):
...
async def new_page(request: Request, current_user: TokenPayload = Depends(get_current_user)):
    return page_templates.TemplateResponse(
...
async def view_page(
    request: Request, record_id: int, current_user: TokenPayload = Depends(get_current_user)
):
    with Session(DB_ENGINE) as db_session:
